...
module.exports = {
    name: "unmute", // Nome principal do comando
    description: "Comando para os moderadores realizarem Desmutes de membros mutados do servidor.", // Descrição do comando
    aliases: ["desmutar"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix) {
        ...
        if (!membro) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você precisa mencionar um membro para desmute!`); // Notifica o autor sobre a falta de menção
        }; // Verifica se o autor mencionou um membro

        if (membro.id == client.user.id) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, porque você está tentando me desmutar? Eu não preciso disso!`); // Notifica o autor sobre não poder desmutar a Mizuhara
        }; // Verifica se o autor mencionou a Mizuhara

        if (membro.id == message.author.id) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você não pode se **Auto-Desmutar**! Que pessoa estranha...`); // Notifica o autor sobre não poder se mencionar no comando
        ...
        if (!message.guild.member(membro).roles.cache.has(role.id)) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você não pode desmutar o ${membro}, pois ele não está mutado no servidor! Use o comando \`${prefix}mute\` para mutar o membro!`); // Notifica o autor sobre o membro já está desmutado
        }; // Verifica se o membro mencionado já está com o cargo de mute

        if (message.guild.me.roles.highest.position < role.position) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, eu não posso desmutar o ${membro}, pois o cargo de mute é maior que o meu cargo! Mude a posição do cargo para baixo de mim e realize o comando novamente!`); // Notifica o autor sobre a poição dos cargos
        }; // Verifica se o cargo de mute é maior que o da Mizuhara

        message.channel.send(`${emojis.IconComments} **|** ${message.author}, descreva um **Nota** para o membro mutado:`).then(msg => {
                    ...
                    message.channel.send(`${message.author}`, embed).then(c => {
                        ...
                        let accept = c.createReactionCollector(collect_accept)
                            .on("collect", async m => {
                                ...
                                try {
                                    membro.roles.remove(role); // Realiza a remoção do cargo de mute
                                } catch (error) {
                                    return message.channel.send(`${emojis.IconCross} **|** ${message.author}, ocorreu um erro ao realizar a remover do cargo, verifique se o cargo de **Mute** está acima do meu e caso o erro aconteça o mesmo erro, faça um reporte para os meus desenvolvedores sobre esse erro.`); // Caso ocorra um erro na adição do cargo, ele notifica o autor
                                }; // Faz o banimento do membro do servidor

                                const embedlog = new Discord.MessageEmbed()
                                    .setColor(colors.perdon)
                                    .setDescription(`${emojis.IconCall} **|** \`${membro.user.tag}\` foi desmutado no servidor **${message.guild.name}**!\n**Nota**: ${mute_reason}`)
                                    .setFooter(`• Moderador: ${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true, format: 'png', size: 1024 })); // Define a embed com as informações do unmute
                                ...
                                message.channel.send(`${emojis.IconCheckMark} **|** ${message.author}, a sua solicitação de unmute foi realizada com sucesso no servidor! Agradeço por ter me usado para punir esses membros de má conduta!`); // Envia a confimação do banimento do membro
                        ...
                        let deny = c.createReactionCollector(collect_deny)
                            .on("collect", m => {
                                ...
                                message.channel.send(`${emojis.IconCheckMark} **|** ${message.author}, a solicitação de unmute do ${membro} foi recusada com sucesso! Espero que você tenha intimidado ele!`); // Notifica o autor sobre a recusão da punição
                            }); // Cria um coletor com o evento de collect que e acionado 
