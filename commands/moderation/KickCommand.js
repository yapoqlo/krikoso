...
module.exports = {
    name: "kick", // Nome principal do comando
    description: "Comando para moderadores realizarem expulsões de membros.", // Descrição do comando
    aliases: ["expulsar"], // Apelidos do comando
    ...
    MemberPerm: ["KICK_MEMBERS"], // Permissões do membro(autor) para usar o comando
    ClientPerm: ["EMBED_LINKS", "KICK_MEMBERS"], // Permissões da Mizuhara para executar o comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix) {
        ...
        if (!membro.kickable) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você não pode punir o membro, pois esse membro não é **Expulsavel**`); // Notifica o autor sobre o membro não ser expulsavel
        }; // Verifica se o membro é expulsavel do servidor
                    ...
                    message.channel.send(`${message.author}`, embed).then(c => {
                        ...
                        let accept = c.createReactionCollector(collect_accept)
                            .on("collect", async m => {
                                ...
                                try {
                                    membro.kick({ reason: `${kick_reason}` }); // Realiza a expulsão colocando o motivo que o autor enviou
                                } catch (error) {
                                    return message.channel.send(`${emojis.IconCross} **|** ${message.author}, ocorreu um erro ao realizar a expulsão, verifique as minhas permissões e caso o erro aconteça o mesmo erro, faça um reporte para os meus desenvolvedores sobre esse erro.`); // Caso ocorra um erro no banimento, e notifica o autor sobre o erro
                                }; // Faz o banimento do membro do servidor

                                const embedlog = new Discord.MessageEmbed()
                                    .setColor(colors.punish)
                                    .setDescription(`${emojis.BanHammet} **|** \`${membro.user.tag}\` foi expulso do servidor **${message.guild.name}**!\n**Motivo**: ${kick_reason}`)
                                    .setFooter(`• Moderador: ${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true, format: 'png', size: 1024 })); // Define a embed com as informações da expulsão
                                ...
                                message.channel.send(`${emojis.IconCheckMark} **|** ${message.author}, a sua solicitação de expulsão foi realizada com sucesso no servidor! Agradeço por ter me usado para punir esses membros de má conduta!`); // Envia a confimação da expulsão do membro
                        ...
                        let deny = c.createReactionCollector(collect_deny)
                            .on("collect", m => {
                                ...
                                message.channel.send(`${emojis.IconCheckMark} **|** ${message.author}, a solicitação de expulsão do ${membro} foi recusada com sucesso! Espero que você tenha intimidado ele!`); // Notifica o autor sobre a recusão da punição
                            }); // Cria um coletor com o evento de collect que e acionado 
