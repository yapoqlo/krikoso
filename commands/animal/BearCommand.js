...
module.exports = {
    name: "bear", // Nome principal do comando
    description: "Comando para mostrar imagens de ursos.", // Descrição do comando
    aliases: ["ursos"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix) {
        let data = await fetch(`https://no-api-key.com/api/v1/animals/bear`).then(rep => rep.json()); // Faz a busca da api e envia em json

        message.channel.startTyping(); // Inicia uma string no canal

        const embed = new Discord.MessageEmbed()
            .setColor(colors.default)
            .setDescription(`Caso queira abaixar essa imagem, [Aperte Aqui!](${data.image})`)
            .setImage(data.image)
