...
module.exports = {
    name: "rabbit", // Nome principal do comando
    description: "Comando para mostrar imagens de coelhos.", // Descrição do comando
    aliases: ["coelhos"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix) {
        let data = await fetch(`https://api.bunnies.io/v2/loop/random/?media=gif,png`).then(rep => rep.json()); // Faz a busca da api e envia em json

        message.channel.startTyping(); // Inicia uma string no canal

        const embed = new Discord.MessageEmbed()
            .setColor(colors.default)
            .setDescription(`Caso queira abaixar essa imagem, [Aperte Aqui!](${data.media.gif})`)
