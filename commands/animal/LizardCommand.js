...
module.exports = {
    name: "lizard", // Nome principal do comando
    description: "Comando para mostrar imagens de lagartos.", // Descrição do comando
    aliases: ["lagartos"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix) {
        let data = await fetch(`https://nekos.life/api/v2/img/lizard`).then(rep => rep.json()); // Faz a busca da api e envia em json

        message.channel.startTyping(); // Inicia uma string no canal

        const embed = new Discord.MessageEmbed()
            .setColor(colors.default)
            .setDescription(`Caso queira abaixar essa imagem, [Aperte Aqui!](${data.url})`)
            .setImage(data.url)
