...
module.exports = {
    name: "cat", // Nome principal do comando
    description: "Comando para mostrar imagens de gatos.", // Descrição do comando
    aliases: ["gatos"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix) {
        let data = await fetch(`https://aws.random.cat/meow`).then(rep => rep.json()); // Faz a busca da api e envia em json

        message.channel.startTyping(); // Inicia uma string no canal

        const embed = new Discord.MessageEmbed()
            .setColor(colors.default)
            .setDescription(`Caso queira abaixar essa imagem, [Aperte Aqui!](${data.file})`)
            .setImage(data.file)
