...
module.exports = {
    name: "feed", // Nome principal do comando
    description: "Comando para alimentar um membro.", // Descrição do comando
    aliases: ["alimentar"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix){
        ...
        if (!user) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você precisa mencionar uma pessoa para alimentar!`); // Notifica o autor sobre a falta de menção
        }; // Verifica se o autor colocou a menção

        if (user.id == client.user.id) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, eu não estou com fome agora!`); // Notifica ao autor que não pode mencionar a Mizuhara
        }; // Verifca se a menção é a Mizuhara

        let img = await neko.sfw.feed(); // Busca a imagem na api de imagens
