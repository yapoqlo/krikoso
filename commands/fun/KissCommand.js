...
module.exports = {
    name: "kiss", // Nome principal do comando
    description: "Comando para beijar um membro.", // Descrição do comando
    aliases: ["baijar"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix){
        ...
        if (!user) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você precisa mencionar uma pessoa para chamar você beijar!`); // Notifica o autor sobre a falta de menção
        }; // Verifica se o autor colocou a menção

        if (user.id == client.user.id) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você não pode me beijar, eu já tenho namorado!`); // Notifica ao autor que não pode mencionar a Mizuhara
        }; // Verifca se a menção é a Mizuhara

        let img = await neko.sfw.kiss(); // Busca a imagem na api de imagens
