...
module.exports = {
    name: "baka", // Nome principal do comando
    description: "Comando para chamar os membros de Baka.", // Descrição do comando
    aliases: ["idiota"], // Apelidos do comando
    ...
    async execute(client, message, args, emojis, colors, config, prefix){
        ...
        if (!user) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você precisa mencionar uma pessoa para chamar de Baka!`); // Notifica o autor sobre a falta de menção
        }; // Verifica se o autor colocou a menção

        if (user.id == client.user.id) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, você não pode me chamar de Baka, pois eu não sou!`); // Notifica ao autor que não pode mencionar a Mizuhara
        }; // Verifca se a menção é a Mizuhara

        let img = await neko.sfw.baka(); // Busca a imagem na api de imagens

        const embed = new Discord.MessageEmbed()
            .setColor(colors.default)
            .setDescription(`${message.author} chamou ${user} de Baka!`)
            .setImage(img.url)
