...
module.exports = {
    name: "mchead", // Nome principal do comando
    description: "Comando para prequisar um cabeça do minecraft a sua escolha.", // Descrição do comando
    aliases: ["mcheadminecraft"], // Apelidos do comandos
    ...
    usage: "[Nome da Cabeça]", // Modo de uso para usar o comando
    cooldown: 3, // Tempo de cooldown do comando
    backlist: true, // Apenas para membros que não estão banidos
    async execute(client, message, args, emojis, colors, config, prefix) {
        if (!args[0]) {
            return message.channel.send(`${emojis.IconCross} **|** ${message.author}, escreva o nome da cabeça que você deseja pesquisar!`); // Notifica ao autor que ele não colocou os argumentos
        }; // Verifica se o autor colocou o nome da cabeça

        let embed = new Discord.MessageEmbed()
            .setDescription(`${emojis.IconLink} **|** Caso queira abaixar essa **Cabeça**, [Aperte Aqui](https://mc-heads.net/head/${args[0]})!`)
